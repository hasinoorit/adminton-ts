...
import { RouteLocationNormalizedLoaded, routerKey, useRoute } from "vue-router"
...
<template>
  <router-link :to="to" custom v-slot="slotProps">
    <div :class="{ 'sb-item': true, active: slotProps.isActive }" ref="itemBlock">
      <a
        v-if="hasChild"
        :href="to"
        @click.prevent
        class="sb-item--header"
        ref="itemHeaderEl"
      >
      ...
      <a
        ...
        v-else
        :to="to"
        :class="{ 'sb-item--header': true, active: isActive }"
        ref="itemHeaderEl"
      >
        <div class="sb-item--icon" v-if="slots.icon">
          <slot name="icon" />
...
.sidebar-container:not(.sidebar-condensed:not(.sidebar-floating)) .sb-item--child {
  overflow: hidden;
  margin-left: 10px;
}
